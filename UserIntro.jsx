...
const {chainRequest} = require('./CreateAcc.jsx')
...
router.post('/', async (req, res) => {
  ...
  if (user.step === 0) {
    if (incomingMessage === "create account") {
      ...
      twiml.message('Please type "create" to create a new account or "log in" to log into an existing account.');
    }
  } else if (user.step === 1) {
    ...
    twiml.message(`Hello ${user.username}`);
